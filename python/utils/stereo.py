...
def clamp_roi(roi, img_size):
    x, y, w, h = (int(v) for v in roi)
    max_w, max_h = img_size

    x = min(max(x, 0), max_w)
    y = min(max(y, 0), max_h)
    w = min(max(w, 0), max_w - x)
    h = min(max(h, 0), max_h - y)
    return x, y, w, h


def intersect_rois(roi_a, roi_b, img_size):
    ax, ay, aw, ah = clamp_roi(roi_a, img_size)
    bx, by, bw, bh = clamp_roi(roi_b, img_size)

    left = max(ax, bx)
    top = max(ay, by)
    right = min(ax + aw, bx + bw)
    bottom = min(ay + ah, by + bh)
    return clamp_roi((left, top, right - left, bottom - top), img_size)


def compute_rectification(params):
    ...
    roi_left = clamp_roi(roi1, img_size)
    roi_right = clamp_roi(roi2, img_size)

    return {
        "maps": (map_l1, map_l2, map_r1, map_r2),
        "R1": R1,
        "R2": R2,
        "P1": P1,
        "P2": P2,
        ...
        "roi_left": roi_left,
        "roi_right": roi_right,
        "roi_overlap": intersect_rois(roi_left, roi_right, img_size),
    }


def compute_maps(params):
    return compute_rectification(params)["maps"]
...
def open_camera(candidates, width, height):
    ...
    for candidate in candidates:
        ...
        if actual_w != total_width or actual_h != height:
            ...
            print(
                f"size mismatch via {label}: requested {total_width}x{height}, "
