...
def parse_args():
    ...
    parser.add_argument("-W", "--width", type=int, default=1280,
                        help="Total frame width (side-by-side). Default: 1280")
...
def _split_raw_frame(frame):
    """Split a side-by-side stereo frame into left/right with size metadata.

    Returns (left, right, full_frame_size, single_view_size).
    Unlike utils.split_stereo_frame, this does not validate against a known
    image size — it discovers the size from the raw frame.
    """
...
def capture_images(cap, board, aruco_dict, aruco_detector, args):
    ...
    while captured < args.count:
        ...
        try:
            left, right, actual_frame_size, actual_pair_size = _split_raw_frame(frame)
...
def main():
    ...
    if "capture" in args.mode:
        ...
        cap = open_camera(candidates, args.width, args.height)
